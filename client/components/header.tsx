      ...
      {({ open }) => (
        <>
          <Menu.Button className="focus:outline-none space-x-2">
            <div className="inline-block items-center space-x-2">
              ...
              {!currentUser.username && (
                <div className="inline-block w-9 h-9 rounded-full bg-gray-200  dark:bg-gray-800 ring-2 ring-gray-200" />
              )}
              {typeof currentUser.username === "string" && (
                <p className="hidden lg:inline-block text-md text-center font-semibold text-gray-700 dark:text-gray-200 group-hover:text-gray-700 dark:group-hover:text-white subpixel-antialiased">
                  {currentUser.username}
                </p>
              )}
            </div>
            <div className="hidden lg:inline-block">
              <FontAwesomeIcon
                icon={faSortDown}
                className="inline mb-2 w-5 h-7 text-gray-700 dark:text-gray-200"
              />
            </div>
          </Menu.Button>
          <Transition
          ...
          >
            <Menu.Items
              static
              className="origin-top-right absolute -right-2 mt-3 w-max rounded-md shadow-lg bg-white dark:bg-gray-800 ring-1 ring-black ring-opacity-5 focus:outline-none"
            >
              <div className="select-none">
                ...
                <Menu.Item>
                  <a
                    ...
                    className="text-gray-700 dark:text-gray-300 block px-8 py-2 text-sm font-medium tracking-wide hover:bg-gray-50 dark:hover:bg-gray-900 cursor-pointer"
                  >
                    Profile
                  </a>
                </Menu.Item>
...
function SearchResult(props: any) {
  ...
  return (
    <a
      onClick={Navigate}
      className={
        "w-full flex flex-row justify-start items-center space-x-4 text-gray-700 dark:text-gray-200 text-sm font-medium tracking-wide hover:bg-gray-50 cursor-pointer " +
    ...
    >
      ...
      <div className="text-gray-700 dark:text-gray-200 text-sm font-semibold group-hover:text-gray-900 subpixel-antialiased overflow-ellipsis">
...
function SearchList() {
  ...
  return (
    <div className="lg:w-1/4 w-1/2 relative self-center items-center inline-block text-center">
      {" "}
      <form
        className="focus-within:shadow-inner flex flex-row px-4 py-1 items-center justify-between rounded-3xl bg-gray-100 dark:bg-gray-900 border border-gray-200 dark:border-gray-900 focus:outline-none "
        onChange={onChange}
        autoComplete="off"
...
export default function Header() {
  ...
  return (
    <div className="fixed top-0 w-full h-min p-2 shadow flex flex-row m-auto text-center align-baseline justify-between bg-white dark:bg-gray-800 z-50">
      <div className="lg:w-10/12 w-full px-1 lg:px-0 flex flex-row m-auto text-center justify-between">
        <a
          onClick={() => {
            Navigate("Dashboard");
          }}
          className="lg:block hidden text-indigo-500 dark:text-indigo-600 text-4xl font-bold tracking-tight cursor-pointer"
        >
          Strugl
        </a>
        <SearchList />
            ...
            <div className="flex flex-row lg:w-1/6 w-1/3 justify-evenly items-center">
              <a onClick={() => Navigate("Dashboard")} title="Home">
                <div className="self-center space-y-1 cursor-pointer group">
                  <div className="w-9 h-2 bg-gray-600 dark:bg-gray-300 bg-opacity-80 rounded-md transition duration-500 ease-in-out transform-gpu translate-x-0 group-hover:translate-x-1"></div>
                  ...
                  <div className="w-9 h-2 bg-gray-600 dark:bg-gray-300 bg-opacity-90 rounded-md transition duration-500 ease-in-out transform-gpu translate-x-0 group-hover:translate-x-1"></div>
                </div>
              </a>
              <div className="self-center">
                <a onClick={() => Navigate("Explore")} title="Explore">
                  <FontAwesomeIcon
                    icon={faCompass}
                    className="w-9 h-9 text-gray-600 dark:text-gray-300 cursor-pointer transition duration-500 ease-in-out transform-gpu hover:rotate-180 rotate-0"
                  />
                </a>
              </div>
            </div>
