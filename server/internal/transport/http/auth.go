...
func (h Handler) HandleAuth(w http.ResponseWriter, r *http.Request, ps httprouter.Params) {
	...
	if user_id >= 0 {
		...
		cookie := http.Cookie{Name: "token", Value: token, MaxAge: 259200, HttpOnly: true, SameSite: http.SameSiteNoneMode}
...
func (h Handler) HandleLogout(w http.ResponseWriter, r *http.Request, ps httprouter.Params) {
	...
	cookie := http.Cookie{Name: "token", Value: "", MaxAge: 1, HttpOnly: true, SameSite: http.SameSiteNoneMode}
	http.SetCookie(w, &cookie)
}
